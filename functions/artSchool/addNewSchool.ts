...
export default async function createSchool(artSchool: IArtSchool) {
    try {
        const res = schoolSchema.safeParse(artSchool)
        if (!res.success) {
        ...
        await addDoc(collection(db, 'art-schools'), artSchool)
